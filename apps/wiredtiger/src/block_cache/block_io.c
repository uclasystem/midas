...
/*
 * __construct_from_disk --
 *     Read the page content from disks after getting a cache miss. Passed in as a callback function
 * to Midas runtime.
 */
int
__construct_from_disk(void *arg)
{
    struct MidasConstructArg *arg_ = (struct MidasConstructArg *)arg;
    WT_SESSION_IMPL *session = arg_->session;
    WT_ITEM *buf = arg_->buf;
    const uint8_t *addr = arg_->addr;
    size_t addr_size = arg_->addr_size;
    bool found = arg_->found;
    bool blkcache_found = arg_->blkcache_found;
    ...
    bool skip_cache_put, timer;

    /* [Midas] */
    uint64_t miss_cycles_stt, miss_cycles_end;
    cache_pool_t pool = midas_get_global_cache_pool();
    if (!pool) {
        MIDAS_PRINT("Failed to get a Midas cache pool!\n");
    }
    ...
    miss_cycles_stt = get_cycles_stt();
    /* Read the block. */
    if (!found) {
     ...
     */
    dsk = ip->data;
    if (!blkcache_found || (blkcache->type != BLKCACHE_DRAM && blkcache->type != BLKCACHE_MIDAS)) {
    ...
    miss_cycles_end = get_cycles_end();

    /* Store the decrypted, possibly compressed, block in the block_cache. */
    if (!skip_cache_put)
        WT_ERR(__wt_blkcache_put(session, ip, addr, addr_size, false));
    midas_inc_cache_miss(pool);
    midas_record_miss_penalty(pool, miss_cycles_end - miss_cycles_stt, ip->size);
...
__wt_blkcache_read(WT_SESSION_IMPL *session, WT_ITEM *buf, const uint8_t *addr, size_t addr_size)
{
    ...
    bool blkcache_found, expect_conversion, found, skip_cache_put;
    // [Midas]
    struct MidasConstructArg arg;
    cache_pool_t pool = midas_get_global_cache_pool();
    if (!pool) {
        MIDAS_PRINT("Failed to get a Midas cache pool!\n");
    }
    ...
    if (!found && blkcache->type != BLKCACHE_UNCONFIGURED) {
        ...
        if (found) {
            blkcache_found = true;
            if (blkcache->type == BLKCACHE_MIDAS) {
                // Midas: snapshot ip and buf
                const void *ip_data_snap = ip->data;
                size_t ip_size_snap = ip->size;
                const void *buf_data_snap = buf->data;
                size_t buf_size_snap = buf->size;
                ...
                if (!expect_conversion) {
                    ...
                    if (!__midas_buf_set(
                          session, buf, blkcache_item->data, blkcache_item->data_size)) {
                        midas_inc_cache_hit(pool);
                        goto verify; // get buf succesfully
                    }
                    optr = (object_ptr_t)blkcache_item->data;
                    if (midas_soft_ptr_is_victim(optr))
                        midas_inc_cache_victim_hit(pool);
                    // Midas: recover internal states changes
                    buf->data = buf_data_snap;
                    buf->size = buf_size_snap;
                    ip->data = ip_data_snap;
                    ip->size = ip_size_snap;
    ...
    /* [Midas] the cache miss path */
    arg.session = session;
    arg.buf = ip;
    arg.addr = addr;
    arg.addr_size = addr_size;
    arg.found = found;
    arg.blkcache_found = blkcache_found;
    if (blkcache->type == BLKCACHE_MIDAS) {
        // WT_ERR(midas_pool_construct(pool, &arg)); /* Read the block. */
        WT_ERR(__construct_from_disk(&arg)); /* Read the block. */
    } else {
        WT_ERR(__construct_from_disk(&arg)); /* Read the block. */
    }
...
// [Midas] this is the original version without using construct()
int
__wt_blkcache_read_orig(WT_SESSION_IMPL *session, WT_ITEM *buf, const uint8_t *addr, size_t addr_size);
int
__wt_blkcache_read_orig(WT_SESSION_IMPL *session, WT_ITEM *buf, const uint8_t *addr, size_t addr_size)
{
    WT_BLKCACHE *blkcache;
