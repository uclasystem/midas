...
__blkcache_alloc(WT_SESSION_IMPL *session, size_t size, void **retp)
{
    ...
    *(void **)retp = NULL;
    ...
    else if (blkcache->type == BLKCACHE_MIDAS)
        return (__midas_malloc(session, size, retp));
  ...
  WT_BLKCACHE_ITEM **blkcache_retp, bool *foundp, bool *skip_cache_putp)
{
    ...
    *blkcache_retp = NULL;
    ...
    if (blkcache_item != NULL) {
        object_ptr_t optr = (object_ptr_t)blkcache_item->data;
        if (midas_soft_ptr_null(optr)) {
            if (midas_soft_ptr_is_victim(optr))
                midas_inc_cache_victim_hit(midas_get_global_cache_pool());
            blkcache_item->num_references--;
            blkcache_item->freq_rec_counter--;
  ...
  WT_SESSION_IMPL *session, WT_ITEM *data, const uint8_t *addr, size_t addr_size, bool write)
{
    ...
    if (blkcache->type == BLKCACHE_MIDAS) {
        bool succ = false;
        object_ptr_t optr = (object_ptr_t)blkcache_store->data;
        cache_pool_t pool = midas_get_global_cache_pool();
        succ = midas_copy_to_soft(pool, optr, data->data, data->size, 0);
        // MIDAS_PRINTF("cache put data %p size %lu\n", optr, data->size);
        if (!succ) {
            goto err;
        }
    } else {
  ...
  u_int overhead_pct, u_int evict_aggressive, uint64_t full_target, bool cache_on_checkpoint)
{
    ...
    if (type == BLKCACHE_NVRAM) {
        ...
        WT_RET_MSG(session, EINVAL, "NVRAM block cache type requires libmemkind");
#endif
    } else if (type == BLKCACHE_MIDAS) {
        // init midas runtime
        midas_runtime_init(session);
    }
...
#endif
    if (blkcache->type == BLKCACHE_MIDAS) {
        // TODO: destory midas runtime
        midas_runtime_destroy(session);
    }
...
#endif
    } else if (WT_STRING_MATCH("midas", cval.str, cval.len) ||
      WT_STRING_MATCH("MIDAS", cval.str, cval.len)) {
        cache_type = BLKCACHE_MIDAS;
    } else
