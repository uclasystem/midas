...
static void
get_p99_lat(TRACK *track)
{
    uint32_t *buf;
    uint32_t n = track->lb_idx;
    uint32_t n99th = 0;
    if (!track->lat_buf)
        return;
    if (n > LAT_BUF_SIZE)
        n = LAT_BUF_SIZE;
    buf = malloc(sizeof(uint32_t) * n);
    memcpy(buf, track->lat_buf, sizeof(int32_t) * n);
...
monitor(void *arg)
{
    ...
    while (!wtperf->stop) {
        ...
        if (wtperf->workers)
            get_p99_lat(&wtperf->workers->read);
  ...
  WT_THREAD_CALLBACK (*func)(void *))
{
    ...
    for (i = 0, thread = base; i < num; ++i, ++thread) {
        ...
        /* [Midas] reset thread track latency ring buffer */
        thread->backup.lat_buf = NULL;
        thread->ckpt.lat_buf = NULL;
        thread->flush.lat_buf = NULL;
        thread->insert.lat_buf = NULL;
        thread->modify.lat_buf = NULL;
        thread->read.lat_buf = NULL;
        thread->scan.lat_buf = NULL;
        thread->truncate.lat_buf = NULL;
        thread->truncate_sleep.lat_buf = NULL;
        thread->update.lat_buf = NULL;
    }
...
stop_threads(u_int num, WTPERF_THREAD *threads)
{
    ...
    for (i = 0; i < num; ++i, ++threads) {
        ...
        /* [Midas] free thread track latency ring buffer */
        if (threads->backup.lat_buf) {
            free(threads->backup.lat_buf);
            threads->backup.lat_buf = NULL;
        }
        if (threads->ckpt.lat_buf) {
            free(threads->ckpt.lat_buf);
            threads->ckpt.lat_buf = NULL;
        }
        if (threads->flush.lat_buf) {
            free(threads->flush.lat_buf);
            threads->flush.lat_buf = NULL;
        }
        if (threads->insert.lat_buf) {
            free(threads->insert.lat_buf);
            threads->insert.lat_buf = NULL;
        }
        if (threads->modify.lat_buf) {
            free(threads->modify.lat_buf);
            threads->modify.lat_buf = NULL;
        }
        if (threads->read.lat_buf) {
            free(threads->read.lat_buf);
            threads->read.lat_buf = NULL;
        }
        if (threads->scan.lat_buf) {
            free(threads->scan.lat_buf);
            threads->scan.lat_buf = NULL;
        }
        if (threads->truncate.lat_buf) {
            free(threads->truncate.lat_buf);
            threads->truncate.lat_buf = NULL;
        }
        if (threads->truncate_sleep.lat_buf) {
            free(threads->truncate_sleep.lat_buf);
            threads->truncate_sleep.lat_buf = NULL;
        }
        if (threads->update.lat_buf) {
            free(threads->update.lat_buf);
            threads->update.lat_buf = NULL;
        }
    }
