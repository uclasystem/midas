...
typedef struct {
    ...
    /* [Midas] latency profiling */
    #define LAT_BUF_SIZE (1000 * 1000 * 10)
    uint32_t *lat_buf;
    uint32_t lb_idx;
...
/* [Midas] Latency profiling */
static inline void
record_lat(TRACK *track, uint64_t lat)
{
    uint32_t idx;
    if (!track->lat_buf) {
        track->lat_buf = malloc(sizeof(uint32_t) * LAT_BUF_SIZE);
        track->lb_idx = 0;
    }
    idx = __sync_fetch_and_add(&track->lb_idx, 1);
    track->lat_buf[idx % LAT_BUF_SIZE] = lat;
}