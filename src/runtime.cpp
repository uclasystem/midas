...
public:
  PageChunk(uint64_t pid, uint64_t region_id)
    ...
    _shm_obj = std::make_shared<boost::interprocess::shared_memory_object>(
        boost::interprocess::open_only, _shm_name.c_str(), rwmode);
    _shm_region =
        std::make_shared<boost::interprocess::mapped_region>(*_shm_obj, rwmode);
...
private:
  uint64_t _pid;
  uint64_t _region_id;
  std::shared_ptr<boost::interprocess::shared_memory_object> _shm_obj;
  std::shared_ptr<boost::interprocess::mapped_region> _shm_region;
...
public:
  ResourceManager(const std::string &daemon_name = kNameCtrlQ) {
    ...
    _sendq = std::make_shared<boost::interprocess::message_queue>(
        boost::interprocess::create_only, get_sendq_name(_id).c_str(),
        kClientQDepth, sizeof(CtrlMsg));
    _recvq = std::make_shared<boost::interprocess::message_queue>(
        boost::interprocess::create_only, get_recvq_name(_id).c_str(),
        kClientQDepth, sizeof(CtrlMsg));
    connect(daemon_name);
  };
  ~ResourceManager() {
    disconnect();
    boost::interprocess::message_queue::remove(get_sendq_name(_id).c_str());
    boost::interprocess::message_queue::remove(get_recvq_name(_id).c_str());
  }
...
private:
  ...
  std::shared_ptr<boost::interprocess::message_queue> _sendq;
  std::shared_ptr<boost::interprocess::message_queue> _recvq;
...
int ResourceManager::Alloc(size_t size) {
  CtrlMsg msg{.id = _id, .op = CtrlOpCode::ALLOC, .mmsg = {.size = size}};
  ...
  _page_chunk_map.insert(std::make_pair(
      ret_msg.mmsg.region_id, std::make_shared<PageChunk>(_id, ret_msg.mmsg.region_id)));
