...
constexpr static int KPerThdLoad = 5000;
...
private:
  ...
  std::shared_ptr<midas::SyncKV<kNumBuckets>> feat_map;
};

/** initialization & utils */
FeatExtractor::FeatExtractor() : raw_feats(nullptr), nr_imgs(0) {
  feat_map =
      std::make_unique<midas::SyncKV<kNumBuckets>>();
  load_imgs();
  load_feats();
...
bool FeatExtractor::serve_req(const FeatReq &req) {
  ...
  size_t feat_size = 0;
  if (feat_map->get(&md5, sizeof(md5), req.feat, sizeof(Feature))) {
  ...
  feat_map->set(&md5, sizeof(md5), req.feat, sizeof(Feature));
...
int FeatExtractor::warmup_cache() {
  ...
  for (int i = 0; i < nr_imgs; i++) {
    ...
    feat_map->set(&md5, sizeof(md5), feats[i].get(), sizeof(Feature));
  }
  ...
  for (int tid = 0; tid < kNrThd; tid++) {
    thds.push_back(std::thread([&, tid = tid] {
      ...
      for (int i = stt; i < end; i++) {
        ...
        feat_map->set(&md5, sizeof(md5), feats[i % feats.size()].get(),
                      sizeof(Feature));
      }
    }));
  }
